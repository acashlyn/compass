...
const LoginScreen = ({ navigation }) => {
	...
	const [loginErrorMessage, setLoginErrorMessage] = useState(""); // State for error message
	...
	const handleLogin = async () => {
		try {
			...
			if (email.trim() && password.trim() && response.user.uid) {
			...
			setLoginErrorMessage(error.message || "Login failed. Please try again."); // Set error message
			setLoginErrorVisible(true); // Show login error modal
			console.log(error);
		}
	};
	...
	return (
		<KeyboardAvoidingView
			...
			behavior={Platform.OS === "ios" ? "padding" : "height"} // Adjust for Android keyboard
		>
			<ScrollView contentContainerStyle={styles.scrollContainer}>
				...
				<View style={styles.logoContainer}>
					<Image
						source={require("../assets/LSPULogo.png")}
				...
				<View style={styles.formContainer}>
					...
					<TextInput
						style={styles.input}
						placeholder="Password" // Changed from "Student Number" to "Password"
						placeholderTextColor="#666"
						secureTextEntry
						value={password}
				...
				>
					<View style={styles.modalContainer}>
						<View style={styles.modalContent}>
							...
							<Text style={styles.modalText}>Log In Successfully.</Text>
							<TouchableOpacity
					...
					<View style={styles.modalContainer}>
						<View style={styles.modalContent}>
							...
							<Text style={styles.modalText}>
								You're continuing without an account. Some features may not be
								available for you.
							</Text>
							...
							<View style={styles.modalButtons}>
								<TouchableOpacity
									...
									onPress={() => setGuestAlertVisible(false)}
								>
